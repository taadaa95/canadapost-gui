...
## Exact 0.4.3 release-candidate validation

1. Compare the exact size and SHA-256 of every canonical 0.4.3 binary intended for publication with the combined `SHA256SUMS.txt` and release metadata.
...
Do not publish `v0.4.3` until these gates are complete and Kris explicitly approves the exact packages.

...
function actionableMessage({ status, category, diagnosticStage, htmlClassification, redirect, fallback }) {
...
function parseCanadaPostError(options = {}) {
  ...
  const message = redactText(actionableMessage({ status: numericStatus, category, diagnosticStage, htmlClassification: htmlInfo?.classification || '', redirect, fallback }), sensitiveValues);
  const diagnostic = {

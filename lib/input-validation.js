...
function validateCustomerNumber(value, { padToTenDigits = true } = {}) {
  const input = cleanString(value, { max: 30 });
  const valid = /^\d{1,10}$/.test(input);
  return {
    valid,
    normalized: valid && padToTenDigits ? input.padStart(10, '0') : input
  };
}

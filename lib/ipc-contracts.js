...
const CONFIG_KEYS = keySet(`
  rememberSettings guidanceSounds webUsername webPassword trackingClientId trackingClientSecret trackingApiEnvironment
  trackingRequestDelayMs trackingResourceTimeoutMs saveLogin saveUsernameOnly estCustomerNumber
  historyCustomerNumber historyAutoMobo historyMobo historyIncludeNoManifest
  claimStreetNumber claimStreetName claimAddressLine2 claimCity claimProvince claimPostalCode claimBusinessName
  claimContactName claimContactPhone claimContactEmail evidenceRetentionDays setupCompleted setupSafetyAcknowledged locale
  developerMode freshTracking updateUrl
`);
const RUN_KEYS = new Set([...CONFIG_KEYS, ...keySet(`
  fresh browserMode afterSubmitMs betweenClaimsMs maxClaims importHistory expectedClaimCount
  selectedTrackingNumbers selectedClassificationRecords diagnosticMode
  diagnosticConfirmed diagnosticRow structureExport scope submitOptions estWorkgroup estMobo estCategoryGroup estFileTypes
  estFrom estTo historyFrom historyTo
`)]);
...
const CHANNEL_KEYS = Object.freeze({
  ...
  'evidence:open': null,
  'history:list': keySet('limit offset latestOnly'),
  'history:export': new Set(),
  ...
  'diagnostics:preview': keySet('components supportReferenceId'),
  'diagnostics:create': keySet('components supportReferenceId acknowledged'),
  ...
  'browser:syncVisibility': keySet('requestId visible bounds generation reason force requireVisible step3Active browserEnabled placeholderVisible rawDomRect viewport'),
  'browser:setBuiltinBounds': keySet('x y width height'),
  'browser:clearSession': keySet('confirmed resetProfile'),
  'setupAssistant:open': keySet('bounds locale'),
  'setupAssistant:setStep': keySet('stepId credentialState credentialPhase'),
  'setupAssistant:setBounds': keySet('x y width height'),
  'setupAssistant:focus': new Set(),
  'setupAssistant:retry': keySet('bounds'),
  'setupAssistant:back': new Set(),
  'setupAssistant:startOver': keySet('bounds locale'),
  'setupAssistant:useCustomerNumber': keySet('candidateId'),
  'setupAssistant:close': new Set(),
  'setupAssistant:state': new Set(),

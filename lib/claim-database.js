...
function finishRun(dbPath, runId, status, counts = {}, metadata = {}) {
  return withDatabase(dbPath, db => {
    const promotedAt = status === 'complete' && (metadata.statePromoted === true || db.prepare('SELECT 1 FROM classification_records WHERE run_id = ? LIMIT 1').get(Number(runId)))
      ? nowIso()
      : null;
    db.prepare(`
...
function listClaimHistory(dbPath, options = {}) {
  ...
  return withDatabase(dbPath, db => {
    ...
    if (latestOnly) {
      where.push(`COALESCE(CAST(ca.dry_run AS INTEGER), 0) = 0 AND ca.id = (
        SELECT MAX(latest_ca.id) FROM claim_attempts latest_ca
        WHERE latest_ca.shipment_id = ca.shipment_id AND COALESCE(CAST(latest_ca.dry_run AS INTEGER), 0) = 0
      )`);
    }
    const rows = db.prepare(`
      SELECT ca.*, ca.status AS attempt_status, s.tracking_number, s.service_code,
        ...
        CASE WHEN COALESCE(CAST(ca.dry_run AS INTEGER), 0) = 0 AND ca.id = (
          SELECT MAX(latest.id) FROM claim_attempts latest
          WHERE latest.shipment_id = ca.shipment_id AND COALESCE(CAST(latest.dry_run AS INTEGER), 0) = 0

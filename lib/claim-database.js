...
function recordClassificationInDb(db, trackingNumber, classification, input = {}, runId = null) {
  ...
  const shipment = upsertShipmentInDb(db, {
    ...
    if (classification.classification === 'REVIEW_REQUIRED' || classification.classification === 'TRACKING_ERROR') {
      const existing = db.prepare("SELECT id FROM manual_reviews WHERE shipment_id = ? AND status = 'open' ORDER BY id DESC LIMIT 1").get(shipment.id);
      if (!existing) {
        db.prepare(`INSERT INTO manual_reviews (shipment_id, classification_id, status, reason_codes_json, opened_at, created_at, updated_at)
          VALUES (?, ?, 'open', ?, ?, ?, ?)`).run(shipment.id, classificationId, canonicalize(classification.reasonCodes), createdAt, createdAt, createdAt);
      }
      db.prepare("UPDATE shipments SET manual_review_state = 'open' WHERE id = ?").run(shipment.id);
    }
...
function listManualReviews(dbPath, options = {}) {
  const status = clean(options.status || 'open', 32);
  const search = clean(options.search, 256);
  const limit = Math.max(1, Math.min(1000, Number(options.limit || 250)));
  return withDatabase(dbPath, db => {
    const params = [];
    const clauses = [];
    if (status !== 'all') { clauses.push('mr.status = ?'); params.push(status); }
    if (search) { clauses.push('(s.tracking_number LIKE ? OR s.reference_number LIKE ?)'); params.push(`%${search}%`, `%${search}%`); }
    return db.prepare(`SELECT mr.*, s.tracking_number, s.reference_number, s.service_code, s.expected_date,
      s.first_attempt_date, s.delivery_date, s.classification AS automated_classification
      FROM manual_reviews mr JOIN shipments s ON s.id = mr.shipment_id
      ${clauses.length ? `WHERE ${clauses.join(' AND ')}` : ''} ORDER BY mr.updated_at DESC LIMIT ?`).all(...params, limit);
  });
}

function updateManualReview(dbPath, reviewId, action, note = '') {
  const allowed = new Set(['note', 'resolved_eligible', 'resolved_not_eligible', 'resolved_deferred']);
  if (!allowed.has(action)) throw new Error('Unsupported manual-review action.');
  return withDatabase(dbPath, db => transaction(db, () => {
    const current = db.prepare('SELECT * FROM manual_reviews WHERE id = ?').get(Number(reviewId));
    if (!current) throw new Error('Manual review not found.');
    const timestamp = nowIso();
    if (action === 'note') {
      db.prepare('UPDATE manual_reviews SET note = ?, updated_at = ? WHERE id = ?').run(clean(note, 4096), timestamp, current.id);
    } else {
      db.prepare(`UPDATE manual_reviews SET status = 'resolved', resolution = ?, resolution_note = ?, resolved_at = ?, updated_at = ? WHERE id = ?`)
        .run(action, clean(note, 4096), timestamp, timestamp, current.id);
      db.prepare("UPDATE shipments SET manual_review_state = 'resolved', updated_at = ? WHERE id = ?").run(timestamp, current.shipment_id);
    }
    db.prepare(`INSERT INTO audit_events (event_type, shipment_id, entity_type, entity_id, detail_json, created_at)
      VALUES ('manual_review_updated', ?, 'manual_review', ?, ?, ?)`)
      .run(current.shipment_id, current.id, canonicalize({ action, note: clean(note, 4096) }), timestamp);
    return db.prepare('SELECT * FROM manual_reviews WHERE id = ?').get(current.id);
  }));
}
...
function listClassificationQueue(dbPath, classification, options = {}) {
  // Legacy values remain readable for migration safety, while all new
  // classifications use the simplified four-outcome model.
  const allowed = new Set(['LATE_CANDIDATE', 'ON_TIME', 'REVIEW_REQUIRED', 'TRACKING_ERROR', 'AUTO_ELIGIBLE', 'MANUAL_REVIEW', 'NOT_ELIGIBLE', 'INSUFFICIENT_DATA']);
  if (!allowed.has(classification)) throw new Error('Unsupported classification queue.');
  const search = clean(options.search, 256);
  const limit = Math.max(1, Math.min(1000, Number(options.limit || 250)));
  return withDatabase(dbPath, db => db.prepare(`SELECT tracking_number, reference_number, service_code, expected_date,
    first_attempt_date, delivery_date, classification, eligibility_reason, updated_at
    FROM shipments WHERE classification = ? AND (? = '' OR tracking_number LIKE ? OR reference_number LIKE ?)
    ORDER BY expected_date, updated_at DESC LIMIT ?`).all(classification, search, `%${search}%`, `%${search}%`, limit));
}
...
function discardIncompleteTrackingRun(dbPath) {
  return withDatabase(dbPath, db => transaction(db, () => {
    ...
    for (const item of affected) {
      ...
      db.prepare("UPDATE manual_reviews SET status = 'discarded', resolved_at = ?, updated_at = ? WHERE classification_id = ? AND status = 'open'")
        .run(nowIso(), nowIso(), item.current_classification_id);
      reverted += 1;
    }

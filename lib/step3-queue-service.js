...
function previewCandidates(dbPath, options = {}) {
  ...
  try {
    ...
    const evaluatedItems = candidateRows(db, run.id).map(row => publicCandidate(row, options, candidateExecutionState(db, row, options)));
    const executionCounts = Object.fromEntries(EXECUTION_STATES.map(state => [state, 0]));
    for (const item of evaluatedItems) executionCounts[item.executionState] = Number(executionCounts[item.executionState] || 0) + 1;
    const items = evaluatedItems.filter(item => item.executable && item.executionState === 'executable');
    return {
      count: items.length,
      executableCount: items.length,
      blockedCount: evaluatedItems.length - items.length,
      excludedCount: evaluatedItems.length - items.length,
      executionCounts,
      items,
      runId: Number(run.id),
...
function selectionForRun(selection) {

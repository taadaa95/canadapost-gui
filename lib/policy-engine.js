...
const POLICY_COVERAGE = Object.freeze({
  from: policyRules.policyVersions.reduce((value, item) => !value || item.effectiveFrom < value ? item.effectiveFrom : value, ''),
  through: policyRules.policyVersions.reduce((value, item) => !value || item.effectiveThrough > value ? item.effectiveThrough : value, '')
});

function policyVersionFor() { return null; }
function peakPeriodFor() { return null; }

function policyGuidanceFor(date) {
  const normalized = normalizeDate(date);
  const covered = Boolean(normalized && normalized >= POLICY_COVERAGE.from && normalized <= POLICY_COVERAGE.through);
  return {
    state: covered ? 'unverified_advisory' : 'policy_review_required',
    covered,
    coverageFrom: POLICY_COVERAGE.from,
    coverageThrough: POLICY_COVERAGE.through
  };
}
...
function resultBase(input, rawInput, options) {
  ...
  const guidance = policyGuidanceFor(applicableExpected || input.shipmentDate);
  ...
  const warningCodes = [];
  warningCodes.push('POLICY_GUIDANCE_UNVERIFIED_ADVISORY');
  if (guidance.state === 'policy_review_required') warningCodes.push('POLICY_REVIEW_REQUIRED');
  if (!input.serviceCode) warningCodes.push('SERVICE_UNKNOWN_ADVISORY');
  else if (!servicePolicy) warningCodes.push('SERVICE_ELIGIBILITY_UNCONFIRMED_ADVISORY');
  else if (servicePolicy.guaranteed === false) warningCodes.push('SERVICE_GUARANTEE_UNVERIFIED_ADVISORY');
  else if (servicePolicy.reviewRequired) warningCodes.push('SERVICE_REVIEW_REQUIRED_ADVISORY');
  if (applicableExpected && !originalExpected) warningCodes.push('ORIGINAL_DELIVERY_STANDARD_UNAVAILABLE_WARNING');
  if (applicableExpected) warningCodes.push('CLAIM_WINDOW_UNVERIFIED_WARNING');
  for (const signal of input.exclusionSignals) {
    warningCodes.push(signal.includes('CLAIM_WINDOW') ? `${signal}_ADVISORY` : `POLICY_ADVISORY_${signal}`);
  }
  ...
  return {
    ...
    policyGuidanceState: guidance.state,
    policyCoverage: guidance,
    ...
    claimSubmissionDeadlineState: claimWindow.ok ? 'unverified_advisory' : 'policy_review_required',
...
function finish(base, classification, reasonCodes, missingEvidence, explanation) {
  ...
  const coreEvidence = {
    ...
    claimSubmissionDeadlineState: '',
    policyGuidanceState: '',
    policyCoverage: {},
    serviceRule: {
      code: result.serviceRule.code,
      name: result.serviceRule.name
    },
    businessDaysRemaining: null,
    calendarDaysRemaining: null,
    peakPeriodStatus: {},
  ...
  result.evidenceHash = sha256Canonical(coreEvidence);

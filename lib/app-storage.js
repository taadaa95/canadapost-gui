...
const CONFIG_FIELDS = new Set([
  ...
  'dryRunDefault', 'setupCompleted', 'setupSafetyAcknowledged', 'locale', 'apiEnvironment', 'trackingApiEnvironment',
...
function saveTrackingApiCredentials(clientId, clientSecret, options = {}) {
  ...
  if (!cleanClientId || !cleanClientSecret || !['production', 'test'].includes(environment)) {
    return { stored: trackingApiCredentialsStored(), backend: credentialBackend(), warning: '' };
  }
  try {
    ...
    return { stored: trackingApiCredentialsStored(), backend: credentialBackend(), warning: `Tracking API credentials could not be saved: ${error.message}` };
  }
}

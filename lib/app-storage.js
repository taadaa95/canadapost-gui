...
const BOOTSTRAP_STATE = userDataBootstrap.getState();
const ISOLATED_OVERRIDE_REQUESTED = Boolean(
  process.env.CANADA_POST_CLAIM_RUNNER_USER_DATA_DIR
  || process.env.CANADA_POST_CLAIM_RUNNER_ISOLATED_TEST_CONFIRM
);
if (ISOLATED_OVERRIDE_REQUESTED && (!BOOTSTRAP_STATE.initialized || !BOOTSTRAP_STATE.active)) {
  throw new Error('The isolated application-data override must be validated before app-storage is imported.');
}
const USER_DATA_ROOT = BOOTSTRAP_STATE.active
  ? BOOTSTRAP_STATE.userDataRoot
  : (process.env.NODE_ENV === 'test' && process.env.CPCR_TEST_USER_DATA
      ? path.resolve(process.env.CPCR_TEST_USER_DATA)
...
const CONFIG_FIELDS = new Set([
  'rememberSettings', 'guidanceSounds', 'webUsername', 'estCustomerNumber',
  'historyCustomerNumber', 'historyAutoMobo',
  ...
  'setupCompleted', 'setupSafetyAcknowledged', 'locale', 'apiEnvironment', 'trackingApiEnvironment',
  'trackingCredentialRevision', 'trackingDiagnosticRevision', 'trackingDiagnosticEnvironment',
  'trackingDiagnosticApiVersion', 'trackingDiagnosticParserVersion', 'trackingDiagnosticSucceededAt',
  'trackingRequestDelayMs', 'trackingResourceTimeoutMs', 'customerNumberPrivacyResetV1'
]);

const LEGACY_CUSTOMER_NUMBER_KEYS = new Set([
  'customernumber', 'estcustomernumber', 'historycustomernumber'
]);
...
function ensureDirs() {
  if (BOOTSTRAP_STATE.active) {
    userDataBootstrap.assertMutablePaths({ USER_DATA_ROOT, DATA_DIR, LOG_DIR, CONFIG_PATH, CREDENTIALS_PATH, CREDENTIAL_KEY_PATH });
  }
...
function sanitizeConfig(input = {}) {
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    else if (key === 'estCustomerNumber') {
      const customerNumber = validateCustomerNumber(value);
      output[key] = customerNumber.valid ? customerNumber.normalized : String(value).slice(0, 30).trim();
...
function redactCustomerNumbers(value, candidates = []) {
  let text = String(value || '');
  for (const candidate of candidates) {
    const customerNumber = String(candidate || '').trim();
    if (customerNumber) text = text.split(customerNumber).join('[REDACTED_CUSTOMER_NUMBER]');
  }
  return text;
}

function removeLegacyCustomerNumberLines(filePath) {
  if (!filePath || !fs.existsSync(filePath)) return false;
  const source = fs.readFileSync(filePath, 'utf8');
  const lines = source.split(/\r?\n/).filter(rawLine => {
    const line = rawLine.trim();
    if (!line || line.startsWith(';') || line.startsWith('#') || !line.includes('=')) return true;
    const key = line.slice(0, line.indexOf('=')).trim().toLowerCase().replace(/[-_]/g, '');
    return !LEGACY_CUSTOMER_NUMBER_KEYS.has(key);
  });
  const output = `${lines.join('\n').replace(/\n+$/, '')}\n`;
  if (output === source) return false;
  writeJsonAtomicText(filePath, output);
  return true;
}

function writeJsonAtomicText(filePath, text) {
  ensureDirs();
  const tempPath = `${filePath}.tmp-${process.pid}-${Date.now()}`;
...
function writeCredentialRecord(record) {
  const clean = { version: 4 };
  for (const key of ['webPassword', 'apiUsername', 'apiPassword', 'trackingClientId', 'trackingClientSecret']) {
    if (validCredentialEntry(record[key])) clean[key] = record[key];
  }
  if (['production', 'development'].includes(record.apiEnvironment)) clean.apiEnvironment = record.apiEnvironment;
  if (['production', 'test'].includes(record.trackingApiEnvironment)) clean.trackingApiEnvironment = record.trackingApiEnvironment;
...
function savePassword(password, remember) {
  ...
  if (!remember) {
    ...
    return { stored: false, updated: true, backend: credentialBackend(), warning: '' };
  }
  if (!cleanPassword) {
    return {
  ...
  try {
    ...
    delete record.password;
    writeCredentialRecord(record);
    return { stored: true, updated: true, backend: encrypted.backend, warning: encrypted.warning };
  } catch (error) {
    return {
      stored: passwordStored(),
      updated: false,
...
function apiCredentialEnvironment() {
  const value = String(readCredentialRecord().apiEnvironment || '').trim().toLowerCase();
  return ['production', 'development'].includes(value) ? value : '';
}
...
function saveApiCredentials(username, password, options = {}) {
  ensureDirs();
  const cleanUsername = String(username || '').trim();
  const cleanPassword = String(password || '').trim();
  const environment = ['production', 'development'].includes(String(options.environment || '').trim().toLowerCase())
    ? String(options.environment).trim().toLowerCase()
    : 'production';
  ...
  try {
    ...
    record.apiEnvironment = environment;
    writeCredentialRecord(record);
    return {
      stored: true,
...
function loadTrackingApiCredentials() {
  const record = readCredentialRecord();
  return {
    clientId: decryptCredential(record.trackingClientId || ''),
    clientSecret: decryptCredential(record.trackingClientSecret || '')
  };
}

function trackingApiCredentialEnvironment() {
  const value = String(readCredentialRecord().trackingApiEnvironment || '').trim().toLowerCase();
  return ['production', 'test'].includes(value) ? value : '';
}

function trackingApiCredentialsStored() {
  const credentials = loadTrackingApiCredentials();
  return Boolean(credentials.clientId && credentials.clientSecret);
}

function saveTrackingApiCredentials(clientId, clientSecret, options = {}) {
  ensureDirs();
  const cleanClientId = String(clientId || '').trim();
  const cleanClientSecret = String(clientSecret || '').trim();
  const environment = String(options.environment || '').trim().toLowerCase();
  if (!cleanClientId || !cleanClientSecret || !['production', 'test'].includes(environment)) {
    return { stored: trackingApiCredentialsStored(), updated: false, backend: credentialBackend(), warning: '' };
  }
  try {
    const encryptedClientId = encryptCredential(cleanClientId);
    const encryptedClientSecret = encryptCredential(cleanClientSecret);
    const record = readCredentialRecord();
    record.trackingClientId = encryptedClientId.entry;
    record.trackingClientSecret = encryptedClientSecret.entry;
    record.trackingApiEnvironment = environment;
    writeCredentialRecord(record);
    return {
      stored: true,
      updated: true,
      backend: encryptedClientId.backend,
      warning: encryptedClientId.warning || encryptedClientSecret.warning || ''
    };
  } catch (error) {
    return { stored: trackingApiCredentialsStored(), updated: false, backend: credentialBackend(), warning: `Tracking API credentials could not be saved: ${error.message}` };
  }
}

function clearTrackingApiCredentials() {
  const record = readCredentialRecord();
  delete record.trackingClientId;
  delete record.trackingClientSecret;
  delete record.trackingApiEnvironment;
  writeCredentialRecord(record);
  return { stored: false };
}
...
function parseIniCredentials(filePath) {
  ...
  for (const rawLine of fs.readFileSync(filePath, 'utf8').split(/\r?\n/)) {
    ...
    if (key === 'apiusername' || key === 'username') username = value;
    if (key === 'apipassword' || key === 'password') password = value;
    if (key === 'apienvironment' && ['production', 'development'].includes(value.toLowerCase())) environment = value.toLowerCase();
  }
...
function removeIniCredentialLines(filePath) {
  ...
  const lines = source.split(/\r?\n/).filter(rawLine => {
    ...
    return !['username', 'password', 'apiusername', 'apipassword'].includes(key);
  });
  const output = `${lines.join('\n').replace(/\n+$/, '')}\n`;
  if (output === source) return false;
  try {
...
function migrateLegacyData() {
  ...
  if (rawConfig.customerNumberPrivacyResetV1 !== true) {
    delete rawConfig.estCustomerNumber;
    delete rawConfig.historyCustomerNumber;
    delete rawConfig.customerNumber;
    removeLegacyCustomerNumberLines(dataUserIni);
    rawConfig.customerNumberPrivacyResetV1 = true;
    migrated.push('customer-number-privacy-reset-v1');
  }
  ...
  const apiResult = saveApiCredentials(sourceCredentials.username, sourceCredentials.password, { environment: sourceCredentials.environment || 'production' });
  if (sourceCredentials.environment) writeConfig({ ...readConfig(), apiEnvironment: sourceCredentials.environment });
  if (apiResult.stored) {
    removeIniCredentialLines(dataUserIni);
    if (!BOOTSTRAP_STATE.active) removeIniCredentialLines(legacyUserIni);
...
function publicConfig() {
  ...
  return {
    ...
    trackingApiCredentialsStored: trackingApiCredentialsStored(),
    trackingApiCredentialEnvironment: trackingApiCredentialEnvironment(),
...
module.exports = {
  ...
  writeConfig,
  sanitizeConfig,
  removeLegacyCustomerNumberLines,
  redactCustomerNumbers,
  publicConfig,
  savePassword,
  loadPassword,
  passwordStored,
  ...
  saveTrackingApiCredentials,
  loadTrackingApiCredentials,
  trackingApiCredentialsStored,
  trackingApiCredentialEnvironment,
  clearTrackingApiCredentials,
  credentialBackend,

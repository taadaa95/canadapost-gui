...
const TABLES = Object.freeze({
  ...
  queue_snapshots: `CREATE TABLE queue_snapshots (
    ...
    snapshot_identity TEXT NOT NULL DEFAULT '',
  ...
  claim_duplicate_tombstones: `CREATE TABLE claim_duplicate_tombstones (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  generated_exports: `CREATE TABLE generated_exports (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    shipment_id INTEGER,
...
const SAFE_COLUMN_ADDITIONS = Object.freeze({
  ...
  queue_snapshots: Object.freeze({
    snapshot_identity: "TEXT NOT NULL DEFAULT ''",
    source_run_id: 'INTEGER REFERENCES runs(id) ON DELETE SET NULL'
  }),
  queue_snapshot_items: Object.freeze({
    classification_id: 'INTEGER REFERENCES classification_records(id) ON DELETE SET NULL'
  })
});

const FINAL_REQUIRED_COLUMNS = Object.freeze({
  ...REQUIRED_COLUMNS,
  runs: Object.freeze([...REQUIRED_COLUMNS.runs, 'promoted_at']),
  ...
  queue_snapshots: Object.freeze([...REQUIRED_COLUMNS.queue_snapshots, 'snapshot_identity', 'source_run_id']),
  queue_snapshot_items: Object.freeze([...REQUIRED_COLUMNS.queue_snapshot_items, 'classification_id'])
});
...
const INDEXES = Object.freeze({
  ...
  idx_duplicate_tombstone_hash: Object.freeze({ table: 'claim_duplicate_tombstones', columns: ['tracking_hash'], sql: 'CREATE UNIQUE INDEX idx_duplicate_tombstone_hash ON claim_duplicate_tombstones(tracking_hash)' }),
  idx_generated_exports_shipment: Object.freeze({ table: 'generated_exports', columns: ['shipment_id', 'id'], sql: 'CREATE INDEX idx_generated_exports_shipment ON generated_exports(shipment_id, id)' })
});
...
const MIGRATION_ORDER = Object.freeze([
  ...
  Object.freeze({ version: 8, stage: 'v8_step3_authority_and_privacy', columns: { runs: ['promoted_at'], queue_snapshots: ['snapshot_identity', 'source_run_id'], queue_snapshot_items: ['classification_id'] }, tables: ['claim_duplicate_tombstones', 'generated_exports'] }),
  Object.freeze({ version: 8, stage: 'v8_promoted_run_backfill', statements: ["UPDATE runs SET promoted_at = completed_at WHERE promoted_at IS NULL AND status = 'complete' AND run_type IN ('tracking', 'full') AND EXISTS (SELECT 1 FROM classification_records cr WHERE cr.run_id = runs.id)"] }),
  Object.freeze({ version: 8, stage: 'dependent_indexes', indexes: Object.keys(INDEXES) }),
  Object.freeze({ version: 8, stage: 'immutability_triggers', triggers: Object.keys(TRIGGERS) })
]);

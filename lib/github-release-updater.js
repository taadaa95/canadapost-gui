...
async function downloadUpdate(update, options) {
  ...
  if (fs.existsSync(destination) && fs.statSync(destination).size === update.artifact.bytes && await fileHash(destination) === update.artifact.sha256) return destination;
  fs.rmSync(destination, { force: true });
  fs.rmSync(partial, { force: true });

  const requested = githubUrl(update.releaseAsset.browser_download_url, 'Update package URL');
  const response = await (options.fetchImpl || globalThis.fetch)(requested, {
    redirect: 'follow',
  ...
  stream.on('data', chunk => {
    ...
    options.onProgress?.(received / update.artifact.bytes);
  });
  try {
...
function registerGithubReleaseUpdater({ app, ipcMain, dialog, BrowserWindow, shell, isolated = false, source = UPDATE_SOURCE }) {
  ...
  async function run() {
    if (busy) return { ok: false, error: 'An update operation is already running.' };
    ...
    try {
      ...
      const downloadedPath = await downloadUpdate(update, {
        ...
        onProgress: ratio => progress(ratio, `Canada Post Claim Runner — Downloading ${Math.floor(ratio * 100)}%`)
      });
      progress(-1);
      ...
      if (install.response !== 0 || !install.checkboxChecked) return { ok: true, available: true, downloaded: true, deferred: true };

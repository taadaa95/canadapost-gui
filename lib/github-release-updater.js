...
function releasesPageUrl(source = UPDATE_SOURCE) {
  apiUrl(source);
  return githubUrl(`https://github.com/${source.owner}/${source.repository}/releases`, 'Releases page');
}
...
function registerGithubReleaseUpdater({ app, ipcMain, registerIpcHandler, dialog, BrowserWindow, shell, isolated = false, source = UPDATE_SOURCE, operationCoordinator = defaultCoordinator, localeProvider = () => 'en-CA' }) {
  ...
  async function run() {
    ...
    try {
      ...
      emit({ stage: 'hidden' });
      if (!update.available) {
        await message({ type: 'info', title: localized('update.none.title', {}, 'No update available'), message: localized('update.none.message', { version: current }, 'Version {version} is up to date.'), buttons: [localized('action.ok', {}, 'OK')] });
        return { ok: true, available: false };
      }
      const choice = await message({
        type: 'info',
        title: localized('update.available.title', {}, 'Update available'),
        message: localized('update.available.message', { version: update.version }, 'Version {version} is available.'),
        detail: `${update.notes || localized('update.available.noNotes', {}, 'No release notes were provided.')}\n\n${localized('update.available.source', { source: `GitHub Releases (${source.owner}/${source.repository})` }, 'Source: {source}')}`,
        buttons: [localized('update.available.download', {}, 'Download update'), localized('update.ready.later', {}, 'Later'), localized('update.available.openRelease', {}, 'Open release page')], defaultId: 0, cancelId: 1, noLink: true
      });
      ...
      if (error.code === 'UPDATE_TRUST_NOT_CONFIGURED') {
        const choice = await message({
          type: 'info',
          title: localized('update.manual.title', {}, 'Automatic updates unavailable'),
          message: localized('update.manual.unsignedBeta', { version: current }, 'Automatic updates are unavailable for this unsigned beta build (version {version}).'),
          detail: localized('update.manual.detail', {}, 'Open the GitHub Releases page to download and install the update manually.'),
          buttons: [localized('update.manual.openReleases', {}, 'Open Releases Page'), localized('action.continue', {}, 'Continue')],
          defaultId: 0,
          cancelId: 1,
          noLink: true
        });
        if (choice.response === 0) await shell.openExternal(releasesPageUrl(source).toString());
        return { ok: false, automaticUpdatesAvailable: false, manualDownload: true, error: error.message, code: error.code };
      }
      const detail = error.code === 'UPDATE_REPOSITORY_NOT_READY'
        ? localized('update.failed.repositoryNotReady', {}, 'Create and publish the public repository taadaa95/canadapost-claim-runner-releases before using updates.')
        : error.message;
      await message({
        type: 'error',

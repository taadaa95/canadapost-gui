...
async function replaceAppImage(downloadedPath, expectedHash, env = process.env) {
  ...
  fs.rmSync(backupStaged, { force: true });
  ...
  const syncDirectory = () => syncPath(path.dirname(current));
  try {
    syncPath(staged);
    fs.copyFileSync(current, backupStaged, fs.constants.COPYFILE_EXCL);
    fs.chmodSync(backupStaged, 0o755);
    syncPath(backupStaged);
    fs.renameSync(backupStaged, backup);
    syncDirectory();

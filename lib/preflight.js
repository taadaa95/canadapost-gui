...
function buildPreflightReport(input = {}) {
  ...
  if (include('step3')) {
    ...
    if (input.liveSubmissionRequested) checks.push(check('portal-compatibility', 'Canada Post portal compatibility', input.portalCompatibility?.ok, {
      severity: 'warning',
      message: input.portalCompatibility?.ok ? 'The current portal fingerprint is compatible and recent.' : (input.portalCompatibility?.reason || 'A compatible portal fingerprint is required for live submission.'),
      action: 'Review the inline Step 3 compatibility advisory. An unverified live run requires an explicit Continue Anyway decision.'
    }));

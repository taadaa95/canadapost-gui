...
function buildPreflightReport(input = {}) {
  ...
  if (include('step2')) {
    ...
    checks.push(check('tracking-api-credentials', 'Tracking API client ID and client secret', input.apiCredentialsAvailable, {
      message: input.apiCredentialsAvailable ? 'Current Developer Portal Tracking API credentials are available.' : (input.legacyApiCredentialsAvailable ? 'Only deprecated legacy Developer Program credentials are available.' : 'Tracking API client ID or client secret is missing.'),
      action: 'Create a Developer Portal app with Tracking product access and save its API Key and API Secret. Legacy or EST credentials are not supported by Step 2.'
    }));
  ...
  if (include('step3')) {
    ...
    checks.push(check('claims-queue', 'Eligible claim queue', Number(input.claimCount || 0) > 0, { message: Number(input.claimCount || 0) > 0 ? `${input.claimCount} eligible claim(s) are queued.` : 'No eligible claims are queued.', action: 'Run Step 2 and refresh the Step 3 queue.' }));
    checks.push(check('builtin-browser', 'Built-in browser mode', input.builtinBrowserRequired !== false, { message: 'Step 3 is restricted to the built-in browser.', action: 'Keep built-in browser mode enabled.' }));

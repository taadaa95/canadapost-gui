...
function validateReleaseIdentity(identity, options = {}) {
  ...
  if (identity.branch !== expectedBranch) throw new Error(`Stable release must be built from ${expectedBranch}; current branch is ${identity.branch || '(detached)'}.`);
  if (!/^[a-f0-9]{40}$/i.test(identity.commit)) throw new Error('Stable release source commit is invalid.');
  if (expectedCommit && identity.commit !== expectedCommit) throw new Error(`Stable release checkout ${identity.commit} does not match reviewed commit ${expectedCommit}.`);

...
function createDiagnosticPackage({ destination, appVersion, config, credentialStatus, logDir, dbPath, dependencyStatus, sensitiveValues = [], components = ['system', 'settings'], supportManifest = {} }) {
  ...
  const manifest = {
    ...
    privacy: 'Credentials, tokens, cookies, browser profiles, raw API bodies, screenshots, addresses, contact details and full tracking numbers are excluded.'
  };
  const entries = {
    'manifest.json': strToU8(JSON.stringify(manifest, null, 2) + '\n')
  };
  ...
  if (selected.has('logs')) {
    for (const file of recentFiles(logDir, name => name.endsWith('.log') && !/response|structure/i.test(name), 5)) {

...
function publicSettings(config = {}) {
  const allowed = [
    ...
    'evidenceRetentionDays', 'dryRunDefault'
  ];
...
function writeZip(destination, entries) {
  ...
  fs.writeFileSync(destination, Buffer.from(bytes), { mode: 0o600 });
...
async function createBackup({ dbPath, dataDir, config, destination, appVersion }) {
  const tempDatabase = path.join(os.tmpdir(), `canadapost-backup-${process.pid}-${Date.now()}.sqlite`);
...
function restoreBackup({ source, dbPath, dataDir, configWriter }) {
  ...
  if (bytes.length > 1024 * 1024 * 1024) throw new Error('Backup archive is too large.');
...
function step3DiagnosticEntries(logDir, sensitiveValues = []) {
  ...
  const walk = current => {
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      const relative = path.relative(directory, source).split(path.sep).map(safeName).join('/');
      const content = fs.readFileSync(source, 'utf8');
      entries[`step3-diagnostics/${relative}`] = strToU8(sanitizeText(content, sensitiveValues));
      totalBytes += size;
    }
  };

  walk(directory);
  entries['step3-diagnostics/included-run.txt'] = strToU8(`${path.basename(directory)}\nScreenshots are excluded from the sanitized diagnostic ZIP.\n`);
  return entries;
}

function createDiagnosticPackage({ destination, appVersion, config, credentialStatus, logDir, dbPath, dependencyStatus, sensitiveValues = [] }) {
  ...
  const integrity = claimDb.integrityCheck(dbPath);
  const manifest = {
    format: 'canadapost-claim-runner-diagnostics',
    version: 1,
    createdAt: new Date().toISOString(),
    appVersion,
    ...
    privacy: 'Passwords, API credentials, cookies, browser profiles, addresses, contact details and full tracking numbers are excluded or redacted.'
  };
  const sanitizedHistory = history.map(item => ({
    ...
    message: sanitizeText(item.message, sensitiveValues).slice(0, 1500),
  ...
  const entries = {
    'manifest.json': strToU8(JSON.stringify(manifest, null, 2) + '\n'),
    'system/dependencies.json': strToU8(JSON.stringify(dependencyStatus, null, 2) + '\n'),
    'system/database-integrity.json': strToU8(JSON.stringify(integrity, null, 2) + '\n'),
    'settings/sanitized-settings.json': strToU8(JSON.stringify(diagnosticsSettings(config, credentialStatus), null, 2) + '\n'),
    'history/recent-attempts.json': strToU8(JSON.stringify(sanitizedHistory, null, 2) + '\n'),
    'history/reconciliation.json': strToU8(JSON.stringify(sanitizedReconciliation, null, 2) + '\n'),
    'history/dashboard.json': strToU8(JSON.stringify(dashboard, null, 2) + '\n'),
    ...step3DiagnosticEntries(logDir, sensitiveValues)
  };
  for (const file of recentFiles(logDir, name => name.endsWith('.log'), 5)) {
    const content = fs.readFileSync(file.path, 'utf8').slice(-250000);
    entries[`logs/${safeName(file.name)}`] = strToU8(sanitizeText(content, sensitiveValues));
  }

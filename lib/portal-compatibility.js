...
function gate(latestRun, options = {}) {
  if (!latestRun) return { ok: false, code: 'PORTAL_COMPATIBILITY_REQUIRED', reason: 'Run the Canada Post workflow health check before a live batch.' };
  let metadata;
  ...
  if (latestRun.status !== 'complete' || compatibility.compatible !== true) {
    return { ok: false, code: 'PORTAL_COMPATIBILITY_FAILED', reason: 'The latest portal health check did not confirm all expected live-claim controls.' };
  }
  if (compatibility.fingerprintVersion !== FINGERPRINT_VERSION) {
    return { ok: false, code: 'PORTAL_COMPATIBILITY_STALE', reason: 'The portal health check uses an obsolete compatibility fingerprint.' };
  }
  ...
  if (!Number.isFinite(checkedAt) || checkedAt > now + 60000 || now - checkedAt > Number(options.maxAgeMs || MAX_AGE_MS)) {
    return { ok: false, code: 'PORTAL_COMPATIBILITY_STALE', reason: 'The portal health check is missing or older than seven days.' };
  }

...
function validateIsolatedOverride(options = {}) {
  ...
  const homeDirectory = resolved(options.homeDirectory || os.homedir());
  const repositoryRoot = safeRealpath(options.repositoryRoot || path.resolve(__dirname, '..'));
  if (canonical === filesystemRoot) throw new IsolatedUserDataError('ISOLATED_PATH_FILESYSTEM_ROOT', 'The filesystem root cannot be used as isolated application data.');
  if (canonical === homeDirectory) throw new IsolatedUserDataError('ISOLATED_PATH_HOME', 'The home directory cannot be used as isolated application data.');
  ...
  if (canonical === defaultUserData || containsPath(defaultUserData, canonical)) {
  ...
  if (/(?:^|[\\/])app\.asar(?:\.unpacked)?(?:[\\/]|$)/i.test(canonical)) {
  ...
  for (const rawForbidden of options.forbiddenPaths || []) {
    ...
    const forbidden = resolved(rawForbidden);
    if (canonical === forbidden || containsPath(forbidden, canonical) || containsPath(canonical, forbidden)) {

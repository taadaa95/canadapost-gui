...
(async () => {
  ...
  try {
    ...
    assert.strictEqual(updateSecurity.verifyUpdateMetadata(metadata, { publicKey: keys.publicKey, channel: 'beta', currentVersion: '0.4.0-dev.1' }).ok, true);
    assert.throws(() => updateSecurity.verifyUpdateMetadata(metadata, { channel: 'beta', currentVersion: '0.4.0-dev.1' }), /No trusted update public key/);
    assert.throws(() => updateSecurity.verifyUpdateMetadata(metadata, { publicKey: keys.publicKey, channel: 'stable', currentVersion: '0.4.0-dev.1' }), /cannot install beta/);

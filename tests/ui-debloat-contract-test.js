...
assert.doesNotMatch(main, /ipcMain\.handle\('financial:(?:get|record)'/);
const configLoadHandler = main.match(/ipcMain\.handle\('config:load'[\s\S]*?ipcMain\.handle\('config:save'/)?.[0] || '';
const configSaveHandler = main.match(/ipcMain\.handle\('config:save'[\s\S]*?ipcMain\.handle\('credentials:clearTrackingApi'/)?.[0] || '';
...
const trackingHandler = main.match(/ipcMain\.handle\('tracking:run'[\s\S]*?ipcMain\.handle\('submit:run'/)?.[0] || '';

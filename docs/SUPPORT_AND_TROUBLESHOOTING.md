...
2. Check Claim History for a **Needs attention** record. Resolve an uncertain attempt there before retrying; `REVIEW_REQUIRED`, `TRACKING_ERROR`, and `ON_TIME` classifications cannot enter Step 3.
3. For browser issues, use the Step 3 session controls and clear the browser profile only after confirming that cookies/storage may be removed. Claim history is preserved.
4. Under **Settings → Advanced**, create a previewed support bundle or local crash report, review it for customer information, then share it through the approved support channel. See [Support bundles](SUPPORT_BUNDLES.md).

...
- Run `npm run release:guard`; verify the provenance branch/commit matches the reviewed PR head and every listed metadata hash.
...
- After Authenticode changes the Windows artifact, regenerate its byte size, SHA-256, checksum file, unsigned manifest candidate, size report, SBOM/provenance binding, and independently compare the source commit. Keep each resulting `package-manifest-*.unsigned.json` candidate with its exact matching artifact. On the offline workstation, set `CPCR_UPDATE_PRIVATE_KEY_FILE` to an external key path and run `npm run release:sign-manifest -- <candidate> <package-manifest-platform.json>`.

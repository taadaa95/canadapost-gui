...
The unsigned Linux 0.4.0-beta.1 package built from commit `701362c` measured **131,090,651 bytes**, a reduction of **256,139,609 bytes (66.14%)** from Dev 10. Its unpacked `resources` directory measured 14 MiB, and the package audit accepted 353 `app.asar` entries with all six worker resources present. Windows size remains a CI result and is not claimed locally.

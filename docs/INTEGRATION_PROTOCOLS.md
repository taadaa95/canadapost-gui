...
The detail response is an official **direct JSON object with no wrapper**. It requires `pin`, `activeExists`, `archiveExists`, `signatureImageExists`, and `suppressSignature`. Service is documented at `$.serviceName` and `$.serviceName2`; expected dates are at `$.expectedDeliveryDate` and `$.changedExpectedDate`; events are at `$.significantEvents[*]`. Significant events require `eventDate` and `eventTime` and may carry `eventIdentifier`, `eventDescription`, and `eventTimeZone`. Unknown JSON fields are ignored. Required-field, type, response-shape, or response-PIN mismatches stop classification; missing first-attempt or other eligibility evidence goes to manual review rather than being fabricated. Persisted normalized evidence excludes response locations, references, free-form event descriptions, and raw response fragments.
...
Current credentials are trimmed at save/use boundaries, but only present/missing metadata is displayed. No secret lengths, prefixes, hashes or values are logged. Website and legacy credentials are never copied into current fields. Existing legacy credentials are preserved until explicit user removal, but the public-beta Step 2 path cannot select them and never falls back from OAuth/JSON to Basic/XML. `lib/legacy-tracking-client.js` is disabled unless an internal caller explicitly passes `enabled: true`; no current worker imports it.
...
3. `unknown` and manual review when neither is recognized.

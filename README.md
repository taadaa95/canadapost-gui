...
Use **Open Detailed Diagnostics** in Step 3 to inspect the latest local run. The History tab's **Support bundle** flow previews its manifest and components before export. System integrity and sanitized setting status are selected by default; masked history, sanitized logs, and Step 3 traces require opt in. Credentials, tokens, cookies, browser profiles, raw Tracking API bodies, and screenshots are always excluded. Review the archive before sharing because free-form operational text can contain information that no automated redactor can guarantee to detect. Live batches also require a current versioned portal-compatibility fingerprint from the built-in-browser workflow health check; see [support bundles and portal compatibility](docs/SUPPORT_BUNDLES_AND_PORTAL.md).

...
Use **Open Detailed Diagnostics** in Step 3 to inspect the latest local run. The History tab's **Support bundle** flow previews its manifest and components before export. System integrity and sanitized setting status are selected by default; masked history and metadata-only log and Step 3 inventories require opt in. Credentials, tokens, cookies, browser profiles, raw Tracking API bodies, screenshots, filenames, and free-form operational text are always excluded. Review the archive before sharing. Before every supervised live batch, Step 3 automatically requires a successful built-in-browser portal-compatibility validation and records its versioned fingerprint; see [support bundles and portal compatibility](docs/SUPPORT_BUNDLES_AND_PORTAL.md).

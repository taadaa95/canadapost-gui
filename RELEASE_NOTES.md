...
- Corrects onboarding order: Canada Post creates the Production application and displays its API Key/API Secret before the application dashboard offers **Add API product**. Claim Runner highlights OK only after both local credential fields are populated, never reads remote Password, Access code, API Key, or API Secret values, and keeps all temporary values unsaved until **Save & Finish**.
- Recognizes customer numbers by priority from an explicit Customer number label, the authenticated Developer Portal **BUSINESS (NUMBER)** selector, **Viewing page as BUSINESS (NUMBER)**, or a recognized business summary. Business name and number require confirmation before changing the local setting; an 8-digit Canada Post Web Username and generic header digits are not treated as customer numbers.
- Recognizes and highlights the authenticated current-business link before its menu is open, then waits for the user to open it. Structural parsing anchored by **View organization settings** handles multiline **Viewing page as / BUSINESS / (NUMBER)** markup; the explicit organization-settings and Create-new-app Customer number fields provide authoritative fallback and mismatch checks.
- Keeps callout text outside the highlighted control using right/left/above/below collision checks; when no unobstructed viewport position exists, only the ring/glow remains. The wider guidance panel emphasizes one current action, and the complete settings checklist appears only at Finish.
- Separates the **Verify your identity** method radio list from an actual Security question Answer challenge, excludes hidden Sign in/background controls when authenticated or an overlay is active, and follows the recorded product sequence: **Add API product → accessible Get access to Tracking → exact link Add in the structural confirmation overlay**. The Tracking card is safely scrolled into view; neither access nor Add is clicked automatically.
- Uses one unsaved Setup draft shared with the standard Settings fields. Customer confirmation and local username/password/API credential entry mirror immediately, **Save & Finish** invokes the existing secure settings save once, and closing without success restores the pre-Setup Settings draft.
- Allows approved Canada Post same-tab navigations and redirects to proceed with their original Chromium semantics. The top-level allowlist is not applied to scripts, images, styles, fonts, CAPTCHA, authentication resources, or subframes. Actual popup requests remain a separate guarded path that retains POST data, while third-party top-level destinations remain blocked.
- Every Setup open creates a fresh `WebContentsView` and webContents generation. Close invalidates the generation before removing its listeners, requesting overlay removal, detaching and destroying the view, clearing every transient reference, and releasing the operation lease in `finally`. Old load/classifier/highlight callbacks cannot mutate a new generation, and a bounded startup watchdog replaces permanent **Preparing** with **Canada Post browser could not start**, **Retry browser**, and **Close Setup**; Retry creates another fresh generation.
- Finish replaces the detached native-browser region with an explicit local completion surface, removes target-failure copy and **Can't find it?**, adds completed-step checkmarks, and makes **Save & Finish** the dominant action.
- Post-authentication guidance now waits for Canada Post's Developer Portal shell/navigation churn to become quiescent before opening the business menu. Business confirmation uses a stacked, readable card and changes immediately to **Business confirmed**; page transitions clear stale callouts, and detected values never receive the large pulsing treatment reserved for actions.
- Production app creation now guides **Production → App name → Create** in sequence. Credential capture explicitly hands off **Copy API Key → Paste API Key → Copy API Secret → Paste API Secret → OK** without reading the clipboard or remote credential fields. Product setup marks Tracking 2.0 as required, validates the product and version in Canada's confirmation overlay, and sends an incorrect product back through **Back** instead of allowing it to complete Setup.
- Optional, default-on **Guidance sounds** provide modest generated success/error tones for meaningful guided transitions. The non-sensitive preference is stored in existing config, visual feedback remains available when sound is off, and semantic generation-scoped event IDs prevent DOM refreshes from replaying tones.
- The stable authenticated portal now highlights the customer’s dynamic current-business control and waits for the user to open it; Claim Runner no longer auto-clicks that menu. Remote copy actions hand off to equally prominent animated local API Key/Secret paste targets.
- Product confirmation identity now includes the current topmost overlay, product/version/plan, and a new attempt ID. Back/catalogue return clears the prior payload, so sequences such as Rating → Returns → Tracking are independently validated and repeated selections can notify once per deliberate attempt without DOM-churn repeats.
- Guided Setup adds a real Electron navigation-history **Back** control, debounced off-flow detection for unrelated approved Canada Post pages, and a confirmation-protected **Start over** action that creates a fresh browser generation while preserving the unsaved in-memory credential draft and persistent Canada Post session partition.
- **Guidance sounds** use an accessible On/Off switch inside Guided Setup and a substantially more audible bounded Web Audio envelope. The persisted preference remains in existing non-sensitive config, while the duplicate normal Settings control is omitted. Duplicate API credential capture cards are collapsed into one durable status.
- Step 1 no longer asks for dates. Each run calculates the latest 35 inclusive Toronto calendar dates in the main process at run time, then retains the existing 30-day segment and adaptive-split behavior.
- Step 2 unions the current rolling import with schema-8 database records whose last promoted authoritative state is active and unresolved. Older in-transit shipments remain eligible after 35 days, transient API/parser failures preserve their prior state for retry, successful delivery retires them into ON_TIME or LATE_CANDIDATE using the original Delivery Standard, and confirmed-on-time cache skips remain intact. `tracking.csv` does not become an ever-growing history store.
...
- Makes the Tracking API production-only in the normal application and removes the obsolete environment selector. Older saved environment labels cannot route a normal run to a test or development gateway.
- Simplifies the customer interface by removing the Advanced Settings utility section, Import CSV, Data Folder, Logs, Reset UI, and the routine browser-profile reset control while retaining the underlying workflow, storage, logging, recovery, and persistent browser-session architecture.

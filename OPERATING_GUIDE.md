...
This is the operator guide for application version **0.4.3** on branch `feature/dev11-beta-release-hardening`. Public stable 0.4.0 supports Linux x64 and Windows x64. Version 0.4.3 remains unpublished until Kris validates the exact canonical packages.
...
| Linux artifact | `Canada.Post.Claim.Runner-0.4.3-linux-x86_64.AppImage` |
| Windows artifact | `Canada.Post.Claim.Runner-0.4.3-win-x64.exe` |
| macOS artifact | `Canada.Post.Claim.Runner-0.4.3-mac-universal.dmg` (publish only if Developer ID signed and notarized) |
